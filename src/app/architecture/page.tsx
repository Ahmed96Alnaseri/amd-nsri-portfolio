...
import { slugify, PROJECTS, type Project } from '@/data/architectureProjects';
...
function ProjectCard({ project, index, slug }: { project: Project; index: number; slug: string }) {
  ...
  const img = current === 0
    ? { src: project.coverImage, subtitle: project.images[0]?.subtitle ?? project.title, position: project.images[0]?.position }
